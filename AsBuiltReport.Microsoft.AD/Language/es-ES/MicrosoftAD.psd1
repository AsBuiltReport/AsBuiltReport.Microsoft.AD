...
'@
    ...
    DC = DC
    IP = IP
    MissingSubnetsBP = Ensure that all subnets at each site are properly defined. Missing subnets can prevent clients from using the site's local Domain Controllers.
    ...
    SiteLinkProtectedBP = If the Site Links in your Active Directory are not protected from accidental deletion, your environment can experience disruptions caused by accidental bulk deletion of objects.
    SiteLinkBridges = Site Link Bridges
    SiteLinkBridgesName = Site Link Bridges Name
    SiteLinksCol = Site Links
    SiteLinkBridgesProtectedBP = If the Site Link Bridges in your Active Directory are not protected from accidental deletion, your environment can experience disruptions caused by accidental bulk deletion of objects.
    SMTPSection = SMTP
...
'@
    ...
    SMBv1BP = SMBv1 is enabled on this system. SMBv1 is an outdated protocol vulnerable to critical security issues, including the EternalBlue exploit. Disable or uninstall SMBv1 on all systems; it has been deprecated and replaced by SMBv2 and SMBv3, which offer improved security features.
    ...
    LDAPCBBindingBP = LDAP channel binding enforcement is not configured on this domain controller. LDAP channel binding is a security feature that protects against man-in-the-middle attacks by ensuring the authenticity and integrity of LDAP connections. Configure LDAP channel binding to require binding on all domain controllers.
    NTLMv1BP = NTLMv1 is enabled on this domain controller. NTLMv1 is an outdated authentication protocol vulnerable to pass-the-hash and credential relay attacks. Disable NTLMv1 on all systems; it has been superseded by NTLMv2, which offers significantly improved security features.
'@
