function Get-AbrADReplicationInfo {
    ...
    process {
        ...
        try {
            ...
            $Connections = Invoke-CommandWithTimeout -Session $DiagramTempPssSession -ScriptBlock { Get-ADReplicationConnection -Filter * -Properties * }
            foreach ($Conn in $Connections) {
                ...
                $AditionalInfo = [PSCustomObject] [ordered] @{
                    $reportTranslate.NewADDiagram.replTransportProtocol = $TransportProtocol
                    $reportTranslate.NewADDiagram.replAutoGenerated = if ($Conn.AutoGenerated) { $reportTranslate.NewADDiagram.replYes } else { $reportTranslate.NewADDiagram.replNo }
                    $reportTranslate.NewADDiagram.replEnabled = if ($Conn.enabledConnection) { $reportTranslate.NewADDiagram.replYes } else { $reportTranslate.NewADDiagram.replNo }
                }
