                        ...
                        foreach ($DC in $DCs) {
                            ...
                            foreach ($Repl in $Replication) {
                                $inObj = [ordered] @{
                                    ...
                                    'Replicate From Directory Server' = Get-ADObject $Repl.ReplicateFromDirectoryServer.Split(",", 2)[1] | Select-Object -ExpandProperty Name
                                    'Replicate To Directory Server' = Get-ADObject $Repl.ReplicateToDirectoryServer | Select-Object -ExpandProperty Name
                        ...
                        foreach ($Fails in $Failures) {
                            Write-PscriboMessage "Collecting Active Directory Sites Replication Failure on '$($Fails.Server)'. (Sites Replication Failure)"
                                $inObj = [ordered] @{
                                    ...
                                    'Partner' =  Get-ADObject $Fails.Partner.Split(",", 2)[1] | Select-Object -ExpandProperty Name
